...
function(input, output, session) {
  ...
  apply_beat <- function(b) {
    if (b < 1 || b > nrow(GUIDED)) return(invisible())
    g <- GUIDED[b, ]
    set_round(g$round)
    bslib::nav_select("nav", g$panel)
    if (!is.na(g$sub)) bslib::nav_select("vitals_sub", g$sub)
  }
  observeEvent(input$mode_guided, { rv_mode("guided") })
  observeEvent(input$mode_free,   { rv_mode("free") })
  observeEvent(rv_mode(), {
    if (rv_mode() == "guided") { rv_beat(1); apply_beat(1) }
    else { rv_beat(0) }
  })
  observeEvent(input$g_next, {
    if (rv_mode() != "guided") return()
    b <- min(nrow(GUIDED), rv_beat() + 1); rv_beat(b); apply_beat(b)
  })
  observeEvent(input$g_prev, {
    if (rv_mode() != "guided") return()
    b <- max(1, rv_beat() - 1); rv_beat(b); apply_beat(b)
  })
  output$g_progress <- renderText({
    b <- rv_beat()
    if (b < 1) "" else sprintf("Beat %d of %d  %s", b, nrow(GUIDED), GUIDED$title[b])
  })

  # ---- module servers -------------------------------------------------------
  introServer("intro",   current_round, set_round,
              go = function(mode, panel) {
                rv_mode(mode)
                if (mode == "free") bslib::nav_select("nav", panel)
              })
  mirrorServer("mirror",  current_round, set_round)
  ...
  verdictServer("verdict", current_round, set_round,
                go = function(panel) bslib::nav_select("nav", panel))
}
