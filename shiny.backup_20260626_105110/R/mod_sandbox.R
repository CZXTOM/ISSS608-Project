        ...
        bslib::card_header(
          htmltools::div(style = "display:flex; align-items:center; justify-content:center; gap:8px; width:100%;",
            ...
            hbtn("Every combination",
              "All 16 fix combinations and the breach-hour zone each produces. Blue, very likely stopped. Amber, lower confidence. Orange, no real effect. The row matching your current switches is highlighted. Your current choice is highlighted."))),
        uiOutput(ns("grid"))
      ),
      bslib::card(
        bslib::card_header(
          htmltools::div(style = "display:flex; align-items:center; justify-content:center; gap:8px; width:100%;",
            htmltools::span("The Judge over time"),
            hbtn("The Judge over time",
              "How many messages the Judge sent each round, coloured by its stance. Restraining means it challenged a move. Permissive means it deferred. Silent means it sent nothing. It did not break, it went quiet at the moments that mattered most."))),
        ggiraph::girafeOutput(ns("posture"), height = "260px")
      )
    )
    )
  )
...
sandboxServer <- function(id, current_round) {
  moduleServer(id, function(input, output, session) {
    ...
    # ---- Judge posture ----
    output$posture <- ggiraph::renderGirafe({
      jp <- judge_posture %>%
        dplyr::mutate(stance = factor(stance, levels = names(STANCE_COL)))
      g <- ggplot2::ggplot(jp, ggplot2::aes(round, n_judge, fill = stance)) +
        ggiraph::geom_col_interactive(
          ggplot2::aes(tooltip = sprintf("%s\n%s\n%d message(s)", round_lab, stance, n_judge),
                       data_id = round), width = 0.7) +
        ggplot2::scale_fill_manual(values = STANCE_COL, name = NULL) +
        ggplot2::labs(x = "round", y = "messages") +
      ...
      ggiraph::girafe(ggobj = g, width_svg = 5.5, height_svg = 2.5,
        options = list(
          ggiraph::opts_tooltip(css = paste0(
            "padding:6px 10px; font-size:12px; color:#2A3439; ",
            "background:#FFFFFF; border:1px solid #C7D0D4; border-radius:6px; ",
